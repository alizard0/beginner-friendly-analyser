...
agent_instructions = [
    ...
    "",
    "## STEP 2: GET CODE CHANGES & ANALYZE",
    ...
    "- CRITICALLY ANALYZE the code for potential bugs, security issues, edge cases, and code quality problems.",
    "- Think like a code reviewer: question assumptions, look for what could go wrong.",
    "- If the changes reference existing functions/variables, consider if you need to use `get_file_content` to see the full context.",
    "",
    "## STEP 3: SEARCH FOR RELATED WORK",
    "Use `search_issues_and_prs` to find similar issues and PRs that might be relevant.",
    "- Extract key terms from the PR title, changed files, and description",
    "- Search for: type:pr OR type:issue + relevant keywords (e.g., file names, feature names, bug types)",
    "- Look for closed/merged PRs that touched similar files or implemented related features",
    "- Look for open/closed issues discussing similar problems or feature requests",
    "- Identify patterns: How were similar problems solved before? What approaches worked?",
    "- Find helpful discussions, decisions, or context from past work",
    "",
    "## STEP 4: GUARD CLAUSES - Skip if:",
    ...
    "",
    "## STEP 5: CALCULATE HEURISTIC SCORE (Threshold: 65 points)",
    ...
    "",
    "## STEP 7: CREATE HELPFUL COMMENT (only if score ≥ 65)",
    ...
    "[Critically analyze the code changes and identify potential problems:",
    "- **Bugs**: Look for logical errors, off-by-one errors, null/undefined handling, type mismatches",
    "- **Edge Cases**: Identify scenarios that might not be handled (empty arrays, null values, boundary conditions)",
    "- **Security**: Check for injection vulnerabilities, exposed secrets, insecure configurations, XSS risks",
    "- **Performance**: Look for inefficient algorithms, N+1 queries, memory leaks, unnecessary computations",
    "- **Error Handling**: Missing try-catch blocks, unhandled promise rejections, no validation",
    "- **Code Quality**: Duplicated code, hardcoded values, magic numbers, unclear variable names",
    "- **Breaking Changes**: API changes that could affect other parts of the codebase",
    "- **Testing**: Missing test cases, untested edge cases, lack of error scenario tests",
    "If everything looks good, say 'No obvious issues detected' but still mention what was checked]",
    "",
    ...
    "",
    "### Review Checklist",
    "[Provide specific, actionable review items based on the actual changes and potential issues identified:",
    "- Verification items for any bugs or issues mentioned above",
    ...
    "",
    "### Related Work",
    "[Based on the search results from similar issues and PRs:",
    "- Link to similar PRs that touched the same files or implemented related features",
    "- Reference relevant issues that discuss the same problem or feature",
    "- Explain how past solutions approached similar problems",
    "- Highlight useful discussions or decisions from previous work",
    "- Suggest: 'You might find these helpful: #123, #456'",
    "- If patterns emerge: 'Similar PRs typically also updated X file' or 'Past implementations used Y approach'",
    "- If no relevant work found, say 'No directly related PRs or issues found']",
    "",
